...
def solve_for_k():
    ...
    accuracy_percentage = (total_accurate/len(trainList))*100

    return [((len(trainList)) - total_accurate), accuracy_percentage]
...
def plotSolve(x_axis_list, y_axis_list):

    plt.plot(x_axis_list, y_axis_list, 'ro')
    plt.axis([0, 32, 0, 100])
    plt.show()
    pass
...
def solve():
    ...
    plotSolve(x_axis_list=K_LIST, y_axis_list= accuracy_percentage)
